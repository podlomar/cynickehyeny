...
export default (context) => async (req, res) => {
  ...
  if (sendingInProgress.has(id)) {
    res.send(render('send.njk', {
      result: { ok: false, status: 'Error', error: 'This newsletter is already being sent.' },
    }));
    return;
  }
  ...
  try {
    ...
    // NOTE: Free plan limits: 500 emails/month and 100 emails/day
    const sentFrom = new Sender("redakce@cynickehyeny.cz", "Cynické hyeny");

    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.write(render('send-start.njk', { total: subscribers.length }));
    ...
    for (const [index, sub] of subscribers.entries()) {
      ...
      try {
        ...
        if (response.statusCode === 202) {
          succeeded++;
          res.write(render('send-item.njk', { index: index + 1, total: subscribers.length, ok: true, email: sub.email }));
        } else {
          console.error(`Failed to send to ${sub.email}: status ${response.statusCode}`);
          failed.push(sub.email);
          res.write(render('send-item.njk', { index: index + 1, total: subscribers.length, ok: false, email: sub.email, reason: `status ${response.statusCode}` }));
        }
      } catch (err) {
        console.error(`Error sending to ${sub.email}:`, err);
        failed.push(sub.email);
        res.write(render('send-item.njk', { index: index + 1, total: subscribers.length, ok: false, email: sub.email, reason: 'error' }));
      }

      // 150ms delay to stay within the Free plan rate limit (10 req/s)
      await new Promise((resolve) => setTimeout(resolve, 150));
    }

    if (test !== 'true' && succeeded > 0) {
      await markAsSent(context, id);
    }

    res.write(render('send-end.njk', {
      status: failed.length === 0 ? 'Success' : 'Partial success',
      succeeded,
